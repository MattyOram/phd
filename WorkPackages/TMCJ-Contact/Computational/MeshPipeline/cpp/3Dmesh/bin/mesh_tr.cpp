...
struct BoneCartField
{
  ...
  enum class CartilageSizingMode {
    N_TETS,
    THICKNESS_OR_MAX
  };

  CartilageSizingMode cartilage_sizing_mode = CartilageSizingMode::N_TETS;
  ...
  FT  max_size   = 0.50; // max target edge length (or circumradius?) within main cartilage region
  ...
  // "Main" cartilage size dictated by the selected cartilage sizing mode.
  ...
  FT cartilage_main_size(FT d_iface, FT d_outer) const {
    ...
    if (cartilage_sizing_mode == CartilageSizingMode::THICKNESS_OR_MAX) {
      return (std::min)(h, max_size);
    }
  ...
  // Smoothed cartilage size: main sizing in interior; within taper band smoothly blend from the near-region size.
  FT cartilage_size_smoothed(const Point_3& p, FT d_iface, FT d_outer) const {
    ...
    // Blend only inside taper region: boundary -> near-region size, at d_taper -> main
    ...
    return lerp(cartilage_near_size(), main, t);
  }
...
int main(int argc, char** argv)
{
  ...
  {
    const std::string cartilage_sizing_mode = s.value("cartilage_sizing_mode", std::string("n_tets"));
    if (cartilage_sizing_mode == "n_tets") {
      field.cartilage_sizing_mode = BoneCartField::CartilageSizingMode::N_TETS;
    } else if (cartilage_sizing_mode == "thickness_or_max") {
      field.cartilage_sizing_mode = BoneCartField::CartilageSizingMode::THICKNESS_OR_MAX;
    } else {
      std::cerr << "ERROR: sizing_field.cartilage_sizing_mode must be 'n_tets' or 'thickness_or_max'\n";
      return 1;
    }
  }
